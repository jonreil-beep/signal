          ...
          {result.role_clusters.map((cluster, i) => (
            <div key={i} className="bg-white rounded-2xl p-5 ring-1 ring-brand-text/8 shadow-sm">
              <div className="flex items-start justify-between gap-3 mb-2.5">
                <h4 className="text-sm font-semibold text-brand-text leading-snug">{cluster.name}</h4>
                <span className={`shrink-0 text-xs font-medium px-2.5 py-1 rounded-full ${CONFIDENCE_STYLES[cluster.confidence]}`}>
                  {cluster.confidence}
                </span>
              </div>
              <p className="text-sm text-brand-text/50 leading-relaxed mb-3">{cluster.reasoning}</p>
              {cluster.signals.length > 0 && (
                <ul className="space-y-1.5">
                  {cluster.signals.map((signal, j) => (
                    <li key={j} className="flex items-start gap-2 text-xs text-brand-text/40">
